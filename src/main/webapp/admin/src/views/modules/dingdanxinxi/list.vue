<template>
  <div class="main-content">
		    ...
		    label="手机号码">
		     <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('dingdanxinxi','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('dingdanxinxi','发送') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="youxixiazaiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'发送':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','发送') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="youxixiazaiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'发送':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','发送') && contents.tableBtnIcon == 0" type="success" size="mini" @click="youxixiazaiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'发送':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <youxixiazai-cross-add-or-update v-if="youxixiazaiCrossAddOrUpdateFlag" :parent="this" ref="youxixiazaiCrossaddOrUpdate"></youxixiazai-cross-add-or-update>
...
import youxixiazaiCrossAddOrUpdate from "../youxixiazai/add-or-update";
export default {
  data() {
    return {
                                                                                                                                                                                                                        searchForm: {
      ...
      addOrUpdateFlag:false,
                  youxixiazaiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            youxixiazaiCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.youxixiazaiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','dingdanxinxi');
      this.$nextTick(() => {
        this.$refs.youxixiazaiCrossaddOrUpdate.init(row.id,type);
      });
    },
                payHandler(row){
